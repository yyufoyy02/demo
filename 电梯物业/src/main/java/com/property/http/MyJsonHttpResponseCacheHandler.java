...
import com.meijialove.utils.MyTextUtil;
...
public abstract class MyJsonHttpResponseCacheHandler extends
    ...
    public void onStart() {
        ...
        if (onJsonCache) {
            if (!MyTextUtil.isEmpty(getRequestURI().toString())) {
                JSONObject mJSONObject = XSimpleACache.get(
                        MJLApplication.mContext).getAsJSONObject(
    ...
    private boolean success(JSONObject response) {
        ...
        JsonElement result_code = rootNode.isJsonObject() ? rootNode
                .getAsJsonObject().get("result_code") : null;
        if (error_code != null) {
            XSimpleLogger.Log().e("error_code.asInt()" + error_code.getAsInt());
            if (error_code.getAsInt() == ErrorCode.SUCCESS
                    || error_code.getAsInt() == ErrorCode.DATA_NOT_FOUND) {
                return true;
            }
        } else if (result_code != null) {
            if (result_code.getAsInt() == ErrorCode.SUCCESS)
                return true;
        }
        return false;
    }
    ...
    public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
        ...
        if (onJsonCache || isCache) {
            if (success(response)) {
                if (cacheTime <= 0) {
                    XSimpleACache.get(MJLApplication.mContext).put(
