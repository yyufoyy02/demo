...
import com.vk.simpleutil.view.PullToRefreshXListView;
...
public class QuickReturnUtils {
	...
	public static int getScrollY(PLA_AbsListView lv) {
		View c = lv.getChildAt(0);
		if (c == null) {
			return 0;
		}
	...
	public static int getScrollY(AbsListView lv) {
		...
		// if(scrollY>0)
		// Log.d("QuickReturnUtils", "getScrollY() : -(c.getTop()) - "+
		// -(c.getTop()));
		// else
		// Log.i("QuickReturnUtils", "getScrollY() : -(c.getTop()) - "+
		// -(c.getTop()));
		...
		for (int i = 0; i < firstVisiblePosition; ++i) {
			...
			// Log.d("QuickReturnUtils",
			// "getScrollY() : sListViewItemHeights.get(i) - "+sListViewItemHeights.get(i));

			if (sListViewItemHeights.get(i) != null) // (this is a sanity check)
				scrollY += sListViewItemHeights.get(i); // add all heights of
														// the views that are
														// gone
	...
	//
	// // Log.d("", "getScrollY() : firstVisiblePosition - " +
	...
	// QuickReturnUtils.dp2px(rv.getContext(), 8)/columnCount);
	// } else {
	// sRecyclerViewItemHeights.put(firstVisiblePosition, c.getHeight());
	// }
	//
	...
	// scrollY += sRecyclerViewItemHeights.get(i); //add all heights of the
	// views that are gone
	// }
	//
	// return scrollY;
	...
	public static int getActionBarHeight(Context context) {
		...
		sActionBarHeight = TypedValue.complexToDimensionPixelSize(
				sTypedValue.data, context.getResources().getDisplayMetrics());
		return sActionBarHeight;
	}
}
