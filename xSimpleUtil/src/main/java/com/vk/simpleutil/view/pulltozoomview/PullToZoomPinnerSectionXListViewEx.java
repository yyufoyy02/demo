package com.vk.simpleutil.view.pulltozoomview;
...
import com.vk.simpleutil.view.PullToRefreshPinnedSectionListView;
 ...
 */
public class PullToZoomPinnerSectionXListViewEx extends PullToZoomBase<PullToRefreshPinnedSectionListView> implements AbsListView.OnScrollListener {
    private static final String TAG = PullToZoomPinnerSectionXListViewEx.class.getSimpleName();
    ...
    public PullToZoomPinnerSectionXListViewEx(Context context) {
        this(context, null);
    }

    public PullToZoomPinnerSectionXListViewEx(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void removeHeaderView() {
        if (mHeaderContainer != null) {
            mRootView.getRefreshableView().removeHeaderView(mHeaderContainer);
        }
    }
     ...
     */
    @Override
    protected PullToRefreshPinnedSectionListView createRootView(Context context, AttributeSet attrs) {
        PullToRefreshPinnedSectionListView lv = new PullToRefreshPinnedSectionListView(context, attrs);
    ...
    private boolean isFirstItemVisible() {
        final Adapter adapter = mRootView.getRefreshableView().getAdapter();
        ...
        if (null == adapter || adapter.isEmpty()) {
             ...
             */
            if (mRootView.getRefreshableView().getFirstVisiblePosition() <= 1) {
        ...
        public void run() {
            if (mZoomView != null) {
                ...
                if ((!mIsFinished) && (mScale > 1.0D)) {
                    ...
                    f2 = mScale - (mScale - 1.0F) * PullToZoomPinnerSectionXListViewEx.sInterpolator.getInterpolation(f1);
