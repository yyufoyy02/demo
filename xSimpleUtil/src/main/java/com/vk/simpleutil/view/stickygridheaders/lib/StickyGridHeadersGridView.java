...
package com.vk.simpleutil.view.stickygridheaders.lib;
...
import com.vk.simpleutil.view.stickygridheaders.lib.StickyGridHeadersBaseAdapterWrapper.HeaderFillerView;
 ...
 */
public class StickyGridHeadersGridView extends HeaderGridView implements
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		for (int i = 0; i < headerPositions.size(); i++) {
			...
			if (frame instanceof FullWidthFixedViewLayout&&getHeaderViewCount() > 0) {
				for (int k = 0; k < headerPositions.size(); k++) {
					frame = getChildAt(headerPositions.get(k));
					if (frame instanceof HeaderFillerView) {
						break;
					}
				}
			}
