...
import com.vk.simpleutil.view.pulltorefresh.huewu.pla.lib.internal.PLA_AdapterView.OnItemClickListener;
import com.vk.simpleutil.view.pulltorefresh.internal.EmptyViewMethodAccessor;
import com.vk.simpleutil.view.pulltorefresh.internal.IndicatorLayout;
import com.vk.simpleutil.view.pulltozoomview.PullToZoomXListViewEx;

public abstract class PullToRefreshAdapterZoomXListViewBase<T extends PullToZoomXListViewEx>
    ...
    public PullToRefreshAdapterZoomXListViewBase(Context context) {
        super(context);
    ...
    public PullToRefreshAdapterZoomXListViewBase(Context context,
                                                 Mode mode, AnimationStyle animStyle) {
    ...
    public void onScrollBase(final PLA_AbsListView view,
    ...
    public void onScrollStateChangedBase(final PLA_AbsListView view,
                                         final int state) {
        /**
    ...
    public final void setEmptyView(View newEmptyView) {
        ...
        if (mRefreshableView.getPullRootView() instanceof EmptyViewMethodAccessor) {
            ((EmptyViewMethodAccessor) mRefreshableView.getPullRootView())
                    .setEmptyViewInternal(newEmptyView);
        } else {
            mRefreshableView.getPullRootView().setEmptyView(newEmptyView);
        }
        mEmptyView = newEmptyView;
    }

    public void setOnItemClickListener(OnItemClickListener listener) {
        mRefreshableView.getPullRootView().setOnItemClickListener(listener);
    }
    ...
    private boolean isFirstItemVisible() {
        ...
        final Adapter adapter = mRefreshableView.getPullRootView().getAdapter();
        if (null == adapter || adapter.isEmpty()) {
            if (DEBUG) {
                Log.d(LOG_TAG, "isFirstItemVisible. Empty View.");
            }

            if (mRefreshableView.getPullRootView().getFirstVisiblePosition() <= 1) {
                final View firstVisibleChild = mRefreshableView.getChildAt(0);
                if (firstVisibleChild != null
                        && firstVisibleChild.getVisibility() != View.GONE) {
             ...
             */
            if (mRefreshableView.getPullRootView().getFirstVisiblePosition() <= 1) {
                final View firstVisibleChild = mRefreshableView.getPullRootView().getChildAt(0);
                if (firstVisibleChild != null) {
                    return firstVisibleChild.getTop() >= mRefreshableView.getPullRootView()
                            .getTop();
                }
            }
        }
    ...
    public boolean isLastItemVisible() {
        ...
        if (null == adapter || adapter.isEmpty()) {
            ...
            final View lastVisibleChild = mRefreshableView.getPullRootView()
                    .getChildAt(mRefreshableView.getPullRootView().getChildCount() - 1);
            if (lastVisibleChild != null
                    && lastVisibleChild.getVisibility() != View.GONE) {
                boolean b = lastVisibleChild.getBottom() <= mRefreshableView.getPullRootView()
                        .getBottom();
//                XSimpleLogger.Log().e("aa1:" + lastVisibleChild.getBottom());
//                XSimpleLogger.Log().e("aa2:" + mRefreshableView.getPullRootView()
//                        .getBottom());
//                XSimpleLogger.Log().e("aa3:" + mRefreshableView
//                        .getBottom());
//                XSimpleLogger.Log().e("aa:" + b);
                return lastVisibleChild.getBottom() <= mRefreshableView.getBottom();
            }
            return true;
        } else {
            final int lastItemPosition = mRefreshableView.getPullRootView().getCount() - 1;
//            final int a = mRefreshableView.getBottom();
            final int lastVisiblePosition = mRefreshableView
                    .getPullRootView().getLastVisiblePosition();
            ...
            if (lastVisiblePosition >= lastItemPosition - 1) {
                ...
                // - mRefreshableView.getHeaderViewsCount();
                final View lastVisibleChild = mRefreshableView.getPullRootView()
                        .getChildAt(mRefreshableView.getPullRootView().getChildCount() - 1);
                if (lastVisibleChild != null) {
