...
import com.vk.simpleutil.view.pulltorefresh.internal.RotateLoadingLayout;
import com.vk.simpleutil.view.pulltorefresh.internal.Utils;
import com.vk.simpleutil.view.pulltorefresh.internal.ViewCompat;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout
	...
	static boolean PinnedSection = false;
	...
	public boolean onTouchEventPullToRefresh(MotionEvent event) {
		return false;
	}

	@Override
	public final boolean onTouchEvent(MotionEvent event) {
		onTouchEventPullToRefresh(event);
		...
		switch (event.getAction()) {
			...
			case MotionEvent.ACTION_UP: {
				if (mIsBeingDragged) {
					mIsBeingDragged = false;
					if ((mRefreshType == RefreshType.both)
							|| (mRefreshType == RefreshType.down && getCurrentMode() == Mode.PULL_FROM_START)
							|| (mRefreshType == RefreshType.up && getCurrentMode() == Mode.PULL_FROM_END)) {
	...
	private RefreshType mRefreshType = RefreshType.both;

	public void setRefreshType(RefreshType type) {
		mRefreshType = type;
	}
	...
	public void onSizeChangedPullToRefresh(int w, int h, int oldw, int oldh) {
	}

	@Override
	protected final void onSizeChanged(int w, int h, int oldw, int oldh) {
		onSizeChangedPullToRefresh(w, h, oldw, oldh);
		if (DEBUG) {
	...
	private void addRefreshableView(Context context, final T refreshableView) {
	...
	public void setPinnedSection(boolean PinnedSection) {
		PullToRefreshBase.PinnedSection = PinnedSection;
	}
}
