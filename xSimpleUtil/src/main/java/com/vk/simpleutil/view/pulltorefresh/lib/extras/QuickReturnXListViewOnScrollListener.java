 ...
 */
public class QuickReturnXListViewOnScrollListener implements
		com.vk.simpleutil.view.pulltorefresh.huewu.pla.lib.internal.PLA_AbsListView.OnScrollListener {
	...
	private final boolean mIsSnappable; // Can Quick Return view snap into
										// place?
	...
	public void onScrollStateChanged(PLA_AbsListView view, int scrollState) {
		...
		for (PLA_AbsListView.OnScrollListener listener : mExtraOnScrollListenerList) {
		...
		if (scrollState == SCROLL_STATE_IDLE && mIsSnappable) {
			...
			case HEADER:
				if (-mHeaderDiffTotal > 0 && -mHeaderDiffTotal < midHeader) {
					ObjectAnimator anim = ObjectAnimator.ofFloat(mHeader,
							"translationY", mHeader.getTranslationY(), 0);
				...
				break;
			case FOOTER:
				if (-mFooterDiffTotal > 0 && -mFooterDiffTotal < midFooter) { // slide
																				// up
					ObjectAnimator anim = ObjectAnimator.ofFloat(mFooter,
							"translationY", mFooter.getTranslationY(), 0);
					anim.setDuration(100);
					anim.start();
					mFooterDiffTotal = 0;
				} else if (-mFooterDiffTotal < mMinFooterTranslation
						&& -mFooterDiffTotal >= midFooter) { // slide down
					ObjectAnimator anim = ObjectAnimator.ofFloat(mFooter,
							"translationY", mFooter.getTranslationY(),
	...
	@Override
	public void onScroll(PLA_AbsListView listview, int firstVisibleItem,
			int visibleItemCount, int totalItemCount) {
		// apply extra on scroll listener
		for (PLA_AbsListView.OnScrollListener listener : mExtraOnScrollListenerList) {
			listener.onScroll(listview, firstVisibleItem, visibleItemCount,
					totalItemCount);
		}
		...
		// Log.d(getClass().getSimpleName(),
		// "onScroll() : mMinHeaderTranslation - "+mMinHeaderTranslation);
		// Log.d(getClass().getSimpleName(),
		// "onScroll() : mMinFooterTranslation - "+mMinFooterTranslation);

		if (diff != 0) {
			switch (mQuickReturnViewType) {
			case HEADER:
				if (diff < 0) { // scrolling down
					mHeaderDiffTotal = Math.max(mHeaderDiffTotal + diff,
							mMinHeaderTranslation);
				} else { // scrolling up
			...
			case FOOTER:
				if (diff < 0) { // scrolling down
					mFooterDiffTotal = Math.max(mFooterDiffTotal + diff,
							-mMinFooterTranslation);
				} else { // scrolling up
	...
	public void registerExtraOnScrollListener(
			PLA_AbsListView.OnScrollListener listener) {
	...
	public static class Builder {
		...
		public QuickReturnXListViewOnScrollListener build() {
			return new QuickReturnXListViewOnScrollListener(this);
		}
	}
